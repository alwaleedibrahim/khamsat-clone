...
export default function UserDropDownBox({userName}: Readonly<{userName: string}>) {
  ...
  return (
    <div className=" bg-white text-[#444] py-3 px-4 w-[200px] shadow-2xl border-2 border-gray-200">
      <ul>
        <li className="flex items-center my-1"><Link href={`/ar/profile`} className="flex items-center"><FaUser className="me-3"/>{userName}</Link></li>
        ...
        <hr />
        <li onClick={()=>{handleLogout()}} className="flex items-center  my-1"><FaSignOutAlt className="me-3"/>خروج</li>
      </ul>
    </div>
  );
}
